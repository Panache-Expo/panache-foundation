...
const verifyEntry = async (supabase, body) => {
  ...
  if (!slug) {
    const error = new Error("Entry link is missing.");
  ...
  const { data, error } = await supabase.rpc("verify_panache_360_contestant_password", {
    p_slug: slug,
    p_password: password,
  });
  ...
  if (!contestant) {
    const invalid = new Error("Invalid private password.");
...
export default async function handler(req, res) {
  try {
    ...
    const result = await verifyEntry(getSupabase(), parseBody(req));
    sendJson(res, 200, result);
  } catch (error) {
    const statusCode = error.statusCode || 500;
    console.error("Panache 360 vote-count API error", {
    ...
    sendJson(res, statusCode, {
      message: error.message || "Could not load vote count.",
