...
const verifyContestantPassword = async (supabase, body) => {
  ...
  if (!slug) {
    const error = new Error("Contestant link is missing.");
  ...
  const { data, error } = await supabase.rpc(
    "verify_miss_panache_contestant_password",
    {
      p_slug: slug,
      p_password: password,
    }
  ...
  if (!contestant) {
    const invalid = new Error("Invalid contestant password.");
...
export default async function handler(req, res) {
  try {
    ...
    console.error("Miss Panache contestant vote-count API error", {
    ...
    sendJson(res, statusCode, {
      message: error.message || "Could not load contestant vote count.",
