...
const PANACHE_360_CONTESTANT_VOTES_API_URL =
  import.meta.env.VITE_PANACHE_360_CONTESTANT_VOTES_API_URL ||
  "/api/panache-360-contestant-votes";
...
const Panache360ContestantVotesPage = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(PANACHE_360_CONTESTANT_VOTES_API_URL, {
        method: "POST",
                ...
                {!entry ? (
                  <Card className="mt-8 rounded-[1.5rem] border-black/8 bg-white shadow-none">
                    <CardHeader>
                      ...
                      <CardDescription>This page is private to the contestant who owns this link.</CardDescription>
                    </CardHeader>
                    <CardContent>
                  ...
                  <div className="mt-8 space-y-4">
                    ...
                    <div className="rounded-[1.5rem] border border-black/8 bg-white p-5">
                      <p className="font-sans text-sm font-semibold text-[#171411]">{entry.organization || "Contestant"}</p>
                      <p className="mt-1 font-sans text-sm text-[#171411]/60">{entry.category_name || "Panache 360"}</p>
                    </div>
