...
const PANACHE_DOR_CONTESTANT_VOTES_API_URL =
  import.meta.env.VITE_PANACHE_DOR_CONTESTANT_VOTES_API_URL ||
  "/api/panache-dor-contestant-votes";
...
const PanacheDorContestantVotesPage = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(PANACHE_DOR_CONTESTANT_VOTES_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug, password }),
      });
      const payload = (await response.json().catch(() => null)) as
        | { contestant?: VoteCountEntry; message?: string }
        | null;
                ...
                {!entry ? (
                  <Card className="mt-8 rounded-[1.5rem] border-black/8 bg-white shadow-none">
                    <CardHeader>
                      ...
                      <CardDescription>This page is private to the nominee who owns this link.</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <form className="space-y-4" onSubmit={handleSubmit}>
                        <div>
                          <Label htmlFor="nomineePassword">Password</Label>
                          <Input
                            id="nomineePassword"
                            type="password"
                  ...
                  <div className="mt-8 space-y-4">
                    ...
                    <div className="rounded-[1.5rem] border border-black/8 bg-white p-5">
                      <p className="font-sans text-sm font-semibold text-[#171411]">{entry.organization || "Nominee"}</p>
                      <p className="mt-1 font-sans text-sm text-[#171411]/60">{entry.category_name || "Panache D’or"}</p>
                    </div>

                    <Button type="button" variant="outline" className="h-12 rounded-full border-black/12 bg-white/74 px-7 font-sans text-sm font-semibold text-[#171411] hover:bg-white" onClick={() => setEntry(null)}>
                      Lock again
                    </Button>
                  </div>
                )}
