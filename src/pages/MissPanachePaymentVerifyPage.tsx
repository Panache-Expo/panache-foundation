...
import { missPanacheVotingService } from "@/integrations/supabase/services";
import type { MissPanacheVoteVerifyResponse } from "@/integrations/supabase/services";
import { isBlindVotingActive } from "@/lib/blind-voting";
import {
  getMissPanacheMotivation,
  rankMissPanacheCategoryNominees,
...
const MISS_PANACHE_WHATSAPP_CHANNEL_URL =
...
const MissPanachePaymentVerifyPage = () => {
  const [searchParams] = useSearchParams();
  const [result, setResult] = useState<MissPanacheVoteVerifyResponse | null>(
    null,
  );
    ...
    const verify = async () => {
      if (previewMode && isPreviewAllowed()) {
        setResult({
          ...
          receipt: {
            id: "preview-payment",
            tx_ref: "preview-miss-panache-vote",
      ...
      try {
        const payload = await missPanacheVotingService.verifyCampayVote({
          txRef,
          reference,
        });
        if (mounted) {
  ...
  const receiptMotivation =
    !blindVoting && receipt && receiptCategory
      ? getMissPanacheMotivation(
          rankMissPanacheCategoryNominees(receiptCategory),
          receipt.nominee_id,
        )
      : null;
  ...
  const sharePath = (() => {
    if (receipt?.nominee_slug) {
      return `/panache-expo/miss-panache/contestants/${receipt.nominee_slug}`;
    }
    if (receiptNominee?.slug) {
      return `/panache-expo/miss-panache/contestants/${receiptNominee.slug}`;
    }
    if (receipt?.category_slug) {
      return `/panache-expo/miss-panache/vote?category=${receipt.category_slug}`;
    }
    if (receiptCategory?.slug) {
      return `/panache-expo/miss-panache/vote?category=${receiptCategory.slug}`;
    }
    return "/panache-expo/miss-panache/vote";
  })();
  const shareUrl = `${shareOrigin}${sharePath}`;
  const shareText = receipt
    ? `I just voted for ${receipt.nominee_name} in Miss Panache. ${
        closeRaceShareLine ? `${closeRaceShareLine} ` : ""
      }Help them win by voting too.`
    : "Support your favorite contestant in Miss Panache.";
      ...
      <main className="mx-auto flex min-h-[70vh] max-w-3xl items-center px-6 py-28 md:px-10">
        <section className="w-full rounded-[2rem] border border-black/8 bg-white p-6 text-center shadow-[0_24px_64px_rgba(17,16,14,0.10)] md:p-10">
          {isLoading ? (
            ...
            <>
              ...
              {isSuccess ? (
                <div className="mx-auto mt-5 max-w-md rounded-[1.4rem] border border-[#8241B6]/16 bg-white p-5">
                  <p className="font-sans text-sm font-semibold text-[#171411]">
                    Stay close to Miss Panache updates
                  </p>
                  <p className="mt-2 font-sans text-sm text-[#171411]/64">
                  ...
                  >
                    <a
                      href={MISS_PANACHE_WHATSAPP_CHANNEL_URL}
                      target="_blank"
                      rel="noreferrer"
                    >
          ...
          {/* <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            ...
            >
              <Link to="/panache-expo/miss-panache/vote">Back to contestants</Link>
            </Button>
          </div> */}
        </section>
      </main>
...
export default MissPanachePaymentVerifyPage;
