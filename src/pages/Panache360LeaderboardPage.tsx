...
import Panache360Hero from "@/assets/panache360-2.jpeg";
import {
  Award,
  BarChart3,
  Loader2,
...
type RankedNominee = Panache360AwardNominee & {
  category: Panache360AwardCategory;
};

type RankedCategory = Panache360AwardCategory & {
...
const rankNominees = (categories: Panache360AwardCategory[]) =>
  categories
    .flatMap((category) =>
...
const NomineeLeaderboardRow = ({
  ...
  <article className="grid gap-4 rounded-[1.35rem] border border-black/8 bg-white/78 p-4 md:grid-cols-[auto_74px_1fr_auto] md:items-center">
    ...
    <div className="flex flex-wrap items-center gap-2 md:justify-end">
      ...
      >
        <Link to={`/panache-expo/panache-360/nominees/${nominee.slug}`}>
          Vote securely
        </Link>
      </Button>
    </div>
...
const Panache360LeaderboardPage = () => {
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-6 md:px-10 lg:grid-cols-[0.56fr_0.44fr] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-2xl font-sans text-lg leading-relaxed text-[#171411]/70">
              ...
              are counted from completed verified payments only and count for{" "}
          ...
          {isLoading ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-black/12 bg-white px-6 py-10 text-center">
              ...
              <p className="mt-4 font-sans text-sm text-[#171411]/68">
                No public Panache 360 nominees are available yet.
              </p>
            </div>
          )}
        </section>
...
export default Panache360LeaderboardPage;
