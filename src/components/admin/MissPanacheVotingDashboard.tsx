...
import type {
  ...
  MissPanachePaidPendingPayment,
  MissPanachePaidPendingSummary,
  MissPanacheVotePayment,
} from "@/integrations/supabase/services";
import { useToast } from "@/hooks/use-toast";
import {
  createMissPanacheVotingCategory,
  createMissPanacheVotingNominee,
  fetchMissPanacheVotingDashboard,
  importMissPanacheNomineesCsv,
  reconcileMissPanacheCampayHistory,
  recoverMissPanachePaidPendingVotes,
  scanMissPanachePaidPendingVotes,
  updateMissPanacheVotingCategory,
  updateMissPanacheVotingNominee,
  uploadMissPanacheVotingNomineePhoto,
  verifyPendingMissPanacheCampayVotes,
...
type MissPanacheVotingDashboardProps = {
  accessKey: string;
};
...
const emptyPaidPendingSummary: MissPanachePaidPendingSummary = {
  count: 0,
  total_votes: 0,
  total_amount_xaf: 0,
  sync_available: true,
...
const categoryToDraft = (category: MissPanacheAwardCategory): CategoryDraft => ({
  slug: category.slug,
  name: category.name,
...
const nomineeToDraft = (nominee: MissPanacheAwardNominee): NomineeDraft => ({
...
export const MissPanacheVotingDashboard = ({
  accessKey,
}: MissPanacheVotingDashboardProps) => {
  const { toast } = useToast();
  const [categories, setCategories] = useState<MissPanacheAwardCategory[]>([]);
  ...
  const [payments, setPayments] = useState<MissPanacheVotePayment[]>([]);
  const [paidPendingPayments, setPaidPendingPayments] = useState<
    MissPanachePaidPendingPayment[]
  >([]);
  const [paidPendingSummary, setPaidPendingSummary] =
    useState<MissPanachePaidPendingSummary>(emptyPaidPendingSummary);
  ...
  const loadVoting = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const voting = await fetchMissPanacheVotingDashboard(accessKey);
      ...
      toast({
        title: "Miss Panache voting unavailable",
        description:
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      const result = await createMissPanacheVotingCategory(accessKey, {
      ...
      toast({
        ...
        description: "Miss Panache nominees can now be added here.",
      });
    } catch (error) {
      toast({
  ...
  const handleSaveCategory = async () => {
    ...
    setIsSavingCategory(true);
    try {
      const result = await updateMissPanacheVotingCategory(
        accessKey,
        selectedCategory.id,
        {
      ...
      toast({
        ...
        description: "Miss Panache category details were updated.",
      });
    } catch (error) {
      toast({
  ...
  const handleCreateNominee = async () => {
    ...
    try {
      const result = await createMissPanacheVotingNominee(accessKey, {
  ...
  const handleSaveNominee = async (forcedStatus?: string) => {
    ...
    setIsSavingNominee(true);
    try {
      const result = await updateMissPanacheVotingNominee(
        accessKey,
        selectedNominee.id,
        {
      ...
      toast({
        ...
        description: "Miss Panache nominee details were updated.",
      });
    } catch (error) {
      toast({
  ...
  const handleUploadNomineePhoto = async (
    ...
    try {
      ...
      const upload = await uploadMissPanacheVotingNomineePhoto(accessKey, {
        fileName: file.name,
  ...
  const handleImportCsv = async () => {
    ...
    try {
      const result = await importMissPanacheNomineesCsv(accessKey, csvText);
      refreshFromVoting(result.voting);
      toast({
  ...
  const handleVerifyPendingPayments = async () => {
    ...
    try {
      const result = await verifyPendingMissPanacheCampayVotes(accessKey);
      refreshFromVoting(result.voting);
      toast({
  ...
  const handleReconcilePayments = async (dryRun: boolean) => {
    ...
    try {
      const result = await reconcileMissPanacheCampayHistory(accessKey, {
  ...
  const handleScanPaidPendingVotes = async () => {
    ...
    try {
      const result = await scanMissPanachePaidPendingVotes(accessKey);
      refreshFromVoting(result.voting);
  ...
  const handleRecoverPaidPendingVotes = async () => {
    ...
    try {
      const result = await recoverMissPanachePaidPendingVotes(accessKey);
      refreshFromVoting(result.voting);
        ...
        <div className="rounded-2xl border border-border/60 bg-background p-4">
          <div className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto_auto]">
              <div>
                <Label htmlFor="missPanacheReconcileStart">Start date</Label>
                <Input
                  id="missPanacheReconcileStart"
                  type="date"
              ...
              <div>
                <Label htmlFor="missPanacheReconcileEnd">End date</Label>
                <Input
                  id="missPanacheReconcileEnd"
                  type="date"
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-border/60 p-5">
              ...
              <div className="space-y-4">
                <Label htmlFor="missPanacheCategorySelect">Category</Label>
                <Select value={selectedCategoryId} onValueChange={setSelectedCategoryId}>
                  <SelectTrigger id="missPanacheCategorySelect">
