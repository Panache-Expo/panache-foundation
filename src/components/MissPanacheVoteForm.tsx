...
type MissPanacheVoteFormProps = {
  contestant: MissPanacheAwardNominee;
...
export const MissPanacheVoteForm = ({
  contestant,
  category,
  payment,
}: MissPanacheVoteFormProps) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await missPanacheVotingService.initializeCampayVote({
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 grid gap-4">
        <div>
          <Label htmlFor={`missPanacheVotes-${contestant.id}`}>Number of votes</Label>
          <p className="mt-1 rounded-full bg-white px-3 py-2 font-sans text-xs font-semibold text-[#8241B6]">
            You can vote more than once.
          </p>
          <Input
            id={`missPanacheVotes-${contestant.id}`}
            type="number"
            min={1}
            step={1}
            value={voteCount}
        ...
        <div>
          <Label htmlFor={`missPanacheEmail-${contestant.id}`}>
            Email for receipt, optional
          </Label>
          <Input
            id={`missPanacheEmail-${contestant.id}`}
            type="email"
            value={email}
